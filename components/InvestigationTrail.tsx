...
export function InvestigationTrail({app,items,onClear,onReplay}:{app:App;items:InvestigationEvent[];onClear:()=>void;onReplay?:(target:string)=>void}){
  ...
  return <>
    <button type="button" ref={triggerRef} className="trail-trigger" onClick={()=>setOpen(true)} aria-label="Open exploration history" aria-expanded={open} aria-controls={open ? "investigation-trail" : undefined}><Icon name="history" size={15}/><span>History</span>{items.length>0&&<b>{items.length}</b>}</button>
