...
import { countLabel, nodeDisplayName, type Flow, type Node } from '../lib/lachesis'
...
function pathBehaviorLabel(flow: Flow, nodes: Node[]) {
  ...
  const firstLabel = first ? nodeDisplayName(first) : 'origin unavailable'
  const lastLabel = last ? nodeDisplayName(last) : 'destination unavailable'
...
export function ConvergenceCanvas({ flows: allFlows, nodes, sinkId, selectedId, onSelect, securityMode = true }: Props) {
  ...
  const focusStatus = focusedOnly
    ? `Showing ${countLabel(flows.length, 'path')} containing ${selectedPoint?.node.label || selectedId}.`
  ...
  return <section className="convergence-canvas" aria-label={`Converging ${pathLabel}`} aria-keyshortcuts="ArrowLeft ArrowRight Home End">
    <header className="convergence-bar"><div><span className="canvas-title">Convergence field</span><span className="canvas-count">{countLabel(flows.length, 'path')} · {countLabel(graph.points.size, 'unique node')}</span>{selectedPoint && <span className="convergence-selected" aria-live="polite">Selected · {nodeDisplayName(selectedPoint.node)}</span>}</div><div className="zoom-controls" role="group" aria-label="Graph zoom"><button type="button" onClick={() => setZoom(value => Math.max(.7, Number((value - .1).toFixed(1))))} aria-label="Zoom out"><Icon name="minus" size={13} /></button><output aria-live="polite">{Math.round(zoom * 100)}%</output><button type="button" onClick={() => setZoom(value => Math.min(1.5, Number((value + .1).toFixed(1))))} aria-label="Zoom in"><Icon name="plus" size={13} /></button><button type="button" onClick={() => setZoom(1)}>Reset</button></div></header>
    ...
    <div className="convergence-index-list" aria-label="Graph nodes">{ordered.map((point, index) => <button ref={point.id === selectedId ? selectedRef : undefined} type="button" key={point.id} className={point.id === selectedId ? 'selected' : ''} onClick={() => onSelect(point.id)} title={`${nodeDisplayName(point.node)} · ${nodeLocation(point.node)}`} aria-pressed={point.id === selectedId} aria-current={point.id === selectedId ? 'step' : undefined} aria-label={`${nodeDisplayName(point.node)}, node ${index + 1} of ${ordered.length}, ${[...point.roles].join(' / ')}${scopeLabel(point.node) ? `, ${scopeLabel(point.node)}` : ''}, ${nodeLocation(point.node)}`}><span>{String(index + 1).padStart(2, '0')}</span><b>{nodeDisplayName(point.node)}</b><small>{[...point.roles].join(' · ')} · {scopeLabel(point.node) ? `${scopeLabel(point.node)} · ` : ''}{point.node.file || 'Source unavailable'}:{point.node.line || '—'}</small></button>)}</div>
  </section>
}
