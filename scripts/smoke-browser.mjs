...
try {
 ...
 assert.equal(await page.getByRole("button", { name: "Map", exact: true }).getAttribute("aria-pressed"), "true", "Explore did not default to Map");
  ...
  assert.equal(new URL(page.url()).searchParams.get("mode"), "full", "Full graph mode did not update the URL");
  assert.equal(await page.getByRole("button", { name: /Switch to guided reading mode/i }).count(), 1, "Full graph mode toggle did not update its label");
  await page.getByRole("button", { name: /Switch to guided reading mode/i }).click();
  assert.equal(new URL(page.url()).searchParams.get("mode"), "guided", "Guided mode did not restore from Full graph mode");
 ...
 await explicitModePage.goto(`${base}/?mode=full`, { waitUntil: "networkidle" });
 ...
 assert.equal(new URL(explicitModePage.url()).searchParams.get("mode"), "full", "explicit Full graph URL mode was not preserved");
