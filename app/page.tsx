...
export default function Page() {
  ...
  return <main className="app-shell" id="top" onDragEnter={event=>{event.preventDefault();dragDepth.current+=1;setDragActive(true)}} onDragOver={event=>{event.preventDefault();event.dataTransfer.dropEffect='copy'}} onDragLeave={event=>{event.preventDefault();dragDepth.current=Math.max(0,dragDepth.current-1);if(dragDepth.current===0)setDragActive(false)}} onDragEnd={()=>{dragDepth.current=0;setDragActive(false)}} onDrop={event=>{event.preventDefault();dragDepth.current=0;setDragActive(false);upload(event.dataTransfer.files?.[0])}}>
    ...
    {view!=='home'&&<Intro view={view as Exclude<View,'home'>} app={app} loadState={loadState} isDemo={isDemo} onUpload={()=>fileRef.current?.click()}/>}
    {view==='home'&&<HomeView app={app} isDemo={isDemo} loadState={loadState} onUpload={()=>fileRef.current?.click()} onView={next=>changeView(next)} onFlow={(nextFlow,nextNode)=>{setView('trace');setFlowId(nextFlow);setStepId(nextNode);setInspectorOpen(true);record('Opened priority witness',nextFlow,'from briefing')}} onSink={nextSink=>{setView('investigate');setSinkId(nextSink);record('Focused execution boundary',nextSink,'from briefing')}} onEntry={(nextIndex,nextHop)=>{setView('journey');setEntryIndex(nextIndex);setHopId(nextHop);setInspectorOpen(true);record('Opened request path',app.entries[nextIndex]?.label??'Unknown entry','from briefing')}}/>}
